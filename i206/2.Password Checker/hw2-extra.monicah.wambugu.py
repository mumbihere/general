...
def is_common(pswd):
    ...
    f.close()
    search_results = search(0,len(common_pwds),pswd.lower())
...
class TestPasswordStrengthChecker(unittest.TestCase):
    ...
    def test_search(self):
        ...
        mid_element_search = search(0,len(common_pwds),'jordan') #position 5000. should be found after 1 search
        self.assertEqual(mid_element_search[1],1)

        #Validate return value
        truesearch = search(0,len(common_pwds),'007007')
        falsesearch = search(0,len(common_pwds),'tH1Ka*')
